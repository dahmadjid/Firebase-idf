...
void FirebaseApp::firebaseClientInit(void) {
    ...
    config.url = "https://google.com";  // you have to set this as https link of
                                        // some sort so that it can init
                                        // properly, you cant leave it empty
...
http_ret_t FirebaseApp::performRequest(const char* url,
    ...
    ESP_ERROR_CHECK(esp_http_client_set_post_field(
        FirebaseApp::client, post_field.c_str(), post_field.length()));
    ...
    if (err != ESP_OK || status_code != 200) {
        ESP_LOGE(FIREBASE_APP_TAG,
                 "Error while performing request esp_err_t code=0x%x | "
                 "status_code=%d",
                 (int)err, status_code);
        ESP_LOGE(FIREBASE_APP_TAG,
...
esp_err_t FirebaseApp::getRefreshToken(bool register_account) {
    ...
    if (register_account) {
        http_ret = FirebaseApp::performRequest(
            FirebaseApp::register_url.c_str(), HTTP_METHOD_POST, account_json);
    } else {
        http_ret = FirebaseApp::performRequest(FirebaseApp::login_url.c_str(),
                                               HTTP_METHOD_POST, account_json);
    }
    ...
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
        ...
        ESP_LOGD(FIREBASE_APP_TAG, "Refresh Token=%s",
                 FirebaseApp::refresh_token.c_str());
        return ESP_OK;
    } else {
        return ESP_FAIL;
    }
...
esp_err_t FirebaseApp::getAuthToken() {
    ...
    std::string token_post_data =
        R"({"grant_type": "refresh_token", "refresh_token":")";
    token_post_data += FirebaseApp::refresh_token + "\"}";

    FirebaseApp::setHeader("content-type", "application/json");
    http_ret = FirebaseApp::performRequest(FirebaseApp::auth_url.c_str(),
                                           HTTP_METHOD_POST, token_post_data);
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
        const char* begin = FirebaseApp::local_response_buffer;
        const char* end = begin + strlen(FirebaseApp::local_response_buffer);
        Json::Reader reader;
        ...
        ESP_LOGD(FIREBASE_APP_TAG, "Auth Token=%s",
                 FirebaseApp::auth_token.c_str());
...
//     err = nvs_set_str(my_handle, "refresh",
//     FirebaseApp::refresh_token.c_str()); err = nvs_set_str(my_handle, "auth",
//     FirebaseApp::auth_token.c_str()); err = nvs_commit(my_handle);
//     nvs_close(my_handle);
...
esp_err_t FirebaseApp::registerUserAccount(const user_account_t& account) {
    if (FirebaseApp::user_account.user_email != account.user_email ||
