...
Json::Value RTDB::getData(const char* path) {
    ...
    http_ret_t http_ret =
        this->app->performRequest(url.c_str(), HTTP_METHOD_GET, "");
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
        ...
        ESP_LOGE(RTDB_TAG,
                 "Error while getting data at path %s| esp_err_t=%d | "
                 "status_code=%d",
                 path, (int)http_ret.err, http_ret.status_code);
        ...
        if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
            ...
            ESP_LOGE(RTDB_TAG,
                     "Failed to get data after refreshing token. double check "
                     "account credentials or database rules");
            this->app->clearHTTPBuffer();
            return Json::Value();
        }
    }
...
esp_err_t RTDB::putData(const char* path, const char* json_str) {
    ...
    this->app->setHeader("content-type", "application/json");
    http_ret_t http_ret =
        this->app->performRequest(url.c_str(), HTTP_METHOD_PUT, json_str);
    this->app->clearHTTPBuffer();
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
...
esp_err_t RTDB::postData(const char* path, const char* json_str) {
    ...
    this->app->setHeader("content-type", "application/json");
    http_ret_t http_ret =
        this->app->performRequest(url.c_str(), HTTP_METHOD_POST, json_str);
    this->app->clearHTTPBuffer();
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
...
esp_err_t RTDB::patchData(const char* path, const char* json_str) {
    ...
    this->app->setHeader("content-type", "application/json");
    http_ret_t http_ret =
        this->app->performRequest(url.c_str(), HTTP_METHOD_PATCH, json_str);
    this->app->clearHTTPBuffer();
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
...
esp_err_t RTDB::deleteData(const char* path) {
    ...
    this->app->setHeader("content-type", "application/json");
    http_ret_t http_ret =
        this->app->performRequest(url.c_str(), HTTP_METHOD_DELETE, "");
    this->app->clearHTTPBuffer();
    if (http_ret.err == ESP_OK && http_ret.status_code == 200) {
